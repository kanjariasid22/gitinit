  ...
  .action(async () => {
    await run(async () => {
      ...
      if (hasStaged) {
        ...
        for (const f of s.staged.added)
          console.log(`\tnew file:   ${f}`)
        for (const f of s.staged.modified)
          console.log(`\tmodified:   ${f}`)
        for (const f of s.staged.deleted)
          console.log(`\tdeleted:    ${f}`)
      }

      if (hasUnstaged) {
        console.log('\nChanges not staged for commit:')
        for (const f of s.unstaged.modified)
          console.log(`\tmodified:   ${f}`)
        for (const f of s.unstaged.deleted)
          console.log(`\tdeleted:    ${f}`)
      }
...
branchCmd
  ...
  .action(async (name: string | undefined, opts: { list?: boolean; delete?: string }) => {
    await run(async () => {
      const repo = openRepo()
  ...
  .action(async (target: string) => {
    await run(async () => {
      ...
      if (branch) {
        ...
        const head = await repo.refStore.readHead()
        console.log(
          `HEAD is now at ${head?.slice(0, 7) ?? target}`,
        )
      }
    })
  })
...
program
  ...
  .action(async (opts: { staged?: boolean }) => {
    await run(async () => {
      const repo = openRepo()
      const diffs = opts.staged
        ? await diffStaged(repo)
        : await diff(repo)
